...
# Note: GitHub commit-SHA URLs (40-char hex) are blocked by the *separate*
# HexHighEntropyString detector (also limit=3.0). That detector's threshold
# is out of scope for this PR; tracked for a follow-up.
...
# Provider-format synthetic secrets that the default detector stack catches
# via dedicated regex plugins (AWSKeyDetector, StripeDetector, JwtTokenDetector,
# PrivateKeyDetector) — NOT via Base64HighEntropyString. These prove that
# raising the entropy threshold from 3.0 → 4.0 does not regress the
# real-world recall path for provider-shaped secrets, because the regex
# plugins detect them independently of the entropy threshold.
#
# Values are canonical documentation examples or clearly-fake test fixtures
# that match the upstream detect-secrets regex patterns. They are not real
# secrets and cannot be used to authenticate against any provider.
_PROVIDER_FORMAT_SECRETS = [
    # AWS access key ID — canonical AWS docs example
    ("aws_access_key", 'AWS_ACCESS_KEY_ID="AKIAIOSFODNN7EXAMPLE"'),
    # JWT token — three b64url segments separated by dots, ≥ 2 dots
    (
        "jwt_token",
        '"token": "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0NSIsIm5hbWUiOiJ0ZXN0In0.'
        'fakeSignatureForUnitTestDoNotUseAsRealCredential123456789"',
    ),
    # PEM private key header — JwtTokenDetector / PrivateKeyDetector match
    (
        "private_key_header",
...
@pytest.mark.parametrize("name,content", _PROVIDER_FORMAT_SECRETS)
def test_provider_format_secrets_caught_by_full_default_stack(name, content):
    """Raising the entropy threshold doesn't regress provider-format recall.

    The Base64HighEntropyString plugin is one of ~70 detectors in the default
    config; the others are regex-based (AWSKeyDetector, JwtTokenDetector,
    PrivateKeyDetector, etc.) and operate independently of the entropy limit.
    This test scans with the *full* default config (the one users actually
    get) and asserts each provider-shaped fixture is still detected.
    """
    obj = _ENTERPRISE_SecretDetection()
    detected = obj.scan_message_for_secrets(content)
    assert detected, (
        f"Provider-format fixture {name!r} was not caught by the full default "
        f"detector stack at the new entropy threshold. Content: {content!r}. "
        f"If this fails, the entropy threshold bump may have masked a recall "
        f"regression — investigate which regex plugin should have matched."
    )
